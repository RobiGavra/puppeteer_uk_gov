...
async function getPageContent() {
  ...
  const url =
    "https://webcaps.sandwell.gov.uk/publicaccess/search.do?action=weeklyList";
  ...
  // Set up download behavior using client
  ...
  try {
    ...
    for (let link of links) {
      ...
      const caseDetails = await page.evaluate(() => {
        ...
        const referenceRow = Array.from(
          document.querySelectorAll("#simpleDetailsTable tr")
        ).find(
          (row) => row.querySelector("th")?.innerText.trim() === "Reference"
        );
        if (referenceRow) {
          details.reference =
            referenceRow.querySelector("td")?.innerText.trim() ||
            "No reference found";
        }
        ...
        const addressRow = Array.from(
          ...
          (row) => row.querySelector("th")?.innerText.trim() === "Address"
        );
        if (addressRow) {
          details.address =
            addressRow.querySelector("td")?.innerText.trim() ||
            "No address found";
        }
        ...
        const proposalRow = Array.from(
          ...
          (row) => row.querySelector("th")?.innerText.trim() === "Proposal"
        );
        if (proposalRow) {
          details.proposal =
            proposalRow.querySelector("td")?.innerText.trim() ||
            "No proposal found";
        }
      ...
      for (let row of rows) {
        try {
          ...
          if (cells.length >= 3) {
            const documentType = await cells[2].evaluate((node) =>
              node.innerText.trim()
            );
            const fileLink = await cells[cells.length - 1].$eval(
              "a",
              (a) => a.href
            );
            if (
